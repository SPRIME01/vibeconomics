...
class Mem0Adapter(AbstractMemoryRepository):
    ...
    def search(self, query: MemoryQuery) -> List[MemorySearchResultItem]:
        try:
            ...
            if results_raw and isinstance(results_raw, list): # mem0ai might return a list of results
                for res_item in results_raw:
                    if isinstance(res_item, dict):
                         processed_results.append(
                            MemorySearchResultItem(
                                id=str(res_item.get("id", uuid.uuid4())), # Ensure ID is string
                                text_content=res_item.get("text", ""),
                                metadata=res_item.get("metadata"),
                                score=float(res_item.get("score", 0.0)) # Ensure score is float
                            )
                        )
