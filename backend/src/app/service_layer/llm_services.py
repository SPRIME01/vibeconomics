...
class AbstractLLMService(abc.ABC, Generic[PydanticModelT]):
    ...
    def get_structured_response(
        self,
        prompt: str,
        output_model: Type[PydanticModelT],
        **kwargs: Any
    ) -> PydanticModelT:
        """
...
class DSPyLLMService(AbstractLLMService[PydanticModelT]):
        ...
        try:
            ...
            if hasattr(self.llm_client, 'basic_request'): 
                ...
                with dspy.context(lm=self.llm_client):
                    program = dspy.predict(dspy.Signature("input -> output", "Predict an output based on input."))
                    response = program(input=prompt)
                    output_content = getattr(response, "output", f"LLM response to: {prompt}")
    ...
    def get_structured_response(
        ...
        **kwargs: Any
        ...
        """
        if prompt == "What is 2 + 2?" and output_model.__name__ == "SampleStructuredOutput":
            try:
        ...
        try:
            ...
            raise ValueError(f"Error creating mock for {output_model.__name__} with data {mock_data}: {e}")
